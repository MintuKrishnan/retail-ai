...
import AmpliencePanel from "./AmpliencePanel";
...
export default function ProductModal({ product, onClose }) {
  ...
  return (
    ...
    >
      <div className="
        bg-canvas-2 border border-border-bright rounded-xl w-full max-w-[640px]
      ...
      ">
        ...
        <button
          onClick={onClose}
          aria-label="Close"
          className="sticky top-4 float-right mr-4 mt-4 z-10 w-8 h-8 rounded-full
            bg-canvas-3 border border-border text-ink-muted hover:text-ink
            hover:bg-border flex items-center justify-center text-xs transition-colors"
        >✕</button>
        ...
        <img
          src={product.image}
          alt={product.title}
          onError={(e) => { e.target.src = `https://picsum.photos/seed/${product.id}/640/260`; }}
        ...
        <div className="p-6 flex flex-col gap-4">
          ...
          {/* Title */}
          <h2 className="font-display font-extrabold text-2xl text-ink tracking-tight leading-none">
            {product.title}
          </h2>

          {/* SEO */}
          {product.seo_title && (
            <p className="text-xs text-ink-dim italic">SEO: {product.seo_title}</p>
          )}
          ...
          <div>
            ...
            <p className="text-[14px] text-ink-muted font-light leading-relaxed">
              {product.description || "—"}
            </p>
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {(product.tags || []).map((tag) => (
                <span key={tag}
                  className="bg-canvas-3 border border-border text-ink-muted
          ...
          {/* ── AMPLIENCE INTEGRATION PANEL ── */}
          <AmpliencePanel product={product} />

          {/* Footer */}
          <div className="pt-3 mt-1 border-t border-border flex gap-4 flex-wrap text-[11px] text-ink-dim">
